...
## XXX we don't have .calls defined for AbstractMatrix
## XXX we don't have SymMatrix
## XXX we don't change storage for eig vects
...
#XXX    @test norm(A) == norm(view(A, :, :))
    ...
    @test Set(keys(eigvals(A))) == Set([x-1, x+1]) # Set(eigvals(A)) == Set([x-1, x+1])
    ...
    evecs = eigvecs(M)
    @test evecs[1][3][1] == [1; 0; 0;;] #  evecs[:,1]== [1, 0, 0]
